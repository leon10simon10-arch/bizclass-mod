...
public class SeatEntity extends Entity {
    ...
    public void tick() {
        ...
        // Safety net: if the seat block is gone, or nobody is riding after the first tick, clean up.
        if (this.level().getBlockState(pos).getBlock() != com.leon.bizclass.registry.ModBlocks.BUSINESS_SEAT.get()) {
            standAndRemove();
            return;
        }
    ...
    private void standAndRemove() {
        ...
        if (!this.level().isClientSide && pos != null) {
            SeatBlock.onStandUp(this.level(), pos);
        }
        this.discard();
    }
